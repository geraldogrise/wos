...
function callWindowSqlEditor(id,pathSql){
...
function resizeSqlEditor(button){
...
function resizeWindowSqlEditor(button,callBackfunction){
    id = $(button).closest('.program').attr('id');
...
function openSqlPath(id_Sqleditor,pathSql){
  
 

  pathSql=pathSql.replace(/\#/g, "\\");
  $.ajax({
            type      : 'post',
 
            url       :'system/controller/ioController.php',
 
            data:   'path='+pathSql+"&action=readFile",
                ...
                success : function(retorno){
				      ...
				      if(retorno.is_erro == "true"){
					     callWindowMessage("","error","Error",retorno.msg);
						  ...
						  editor =  $('#'+id_Sqleditor).find('.CodeMirror')[0].CodeMirror;
...
function openSql_O(button,id_program){
...
function saveSql(button,id_program){
