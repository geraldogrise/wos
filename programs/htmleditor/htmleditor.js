 ...
 */
var id_program = "";
var contHtmlEditor = 0;


function callWindowHtmlEditor(id,pathHtml){
...
function resizeHtmlEditor(button){
...
function resizeWindowHtmlEditor(button,callBackfunction){
    id = $(button).closest('.program').attr('id');
...
function openHtmlPath(id_Htmleditor,pathHtml){
  ...
  $.ajax({
            ...
            data:   'path='+pathHtml+"&action=readFile",
                ...
                success : function(retorno){
				      ...
				      if(retorno.is_erro == "true"){
					     callWindowMessage("","error","Error",retorno.msg);
							...
							editor =  $('#'+id_Htmleditor).find('.CodeMirror')[0].CodeMirror;
...
function openHtml_O(button,id_program){
...
function saveHtml(button,id_program){
