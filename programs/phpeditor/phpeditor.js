...
function callWindowPhpEditor(id,pathPhp){
...
function resizePhpEditor(button){
...
function resizeWindowPhpEditor(button,callBackfunction){
    id = $(button).closest('.program').attr('id');
...
function openPhpPath(id_Phpeditor,pathPhp){
  
 

  pathPhp=pathPhp.replace(/\#/g, "\\");
  $.ajax({
            type      : 'post',
 
            url       :'system/controller/ioController.php',
 
            data:   'path='+pathPhp+"&action=readFile",
            ...
            dataType  : 'JSON',
                ...
                success : function(retorno){
				      ...
				      if(retorno.is_erro == "true"){
						 ...
						 editor =  $('#'+id_Phpeditor).find('.CodeMirror')[0].CodeMirror;
...
function openPhp_O(button,id_program){
...
function savePhp(button,id_program){
