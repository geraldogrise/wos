 ...
 */
var id_program = "";
var contJavaEditor = 0;




function callWindowJavaEditor(id,pathJava){
...
function resizeJavaEditor(button){
...
function resizeWindowJavaEditor(button,callBackfunction){
    id = $(button).closest('.program').attr('id');
...
function openJavaPath(id_Javaeditor,pathJava){
  
 

  pathJava=pathJava.replace(/\#/g, "\\");
  $.ajax({
            type      : 'post',
 
            url       :'system/controller/ioController.php',
 
            data:   'path='+pathJava+"&action=readFile",
                ...
                success : function(retorno){
				      ...
				      if(retorno.is_erro == "true"){
					    callWindowMessage("","error","Error",retorno.msg);
						  ...
						  editor =  $('#'+id_Javaeditor).find('.CodeMirror')[0].CodeMirror;
...
function openJava_O(button,id_program){
...
function saveJava(button,id_program){
