 ...
 */
var id_program = "";
var contPythonEditor = 0;




function callWindowPythonEditor(id,pathPython){
...
function resizePythonEditor(button){
...
function resizeWindowPythonEditor(button,callBackfunction){
    id = $(button).closest('.program').attr('id');
...
function openPythonPath(id_Pythoneditor,pathPython){
  
 

  pathPython=pathPython.replace(/\#/g, "\\");
  $.ajax({
            type      : 'post',
 
            url       :'system/controller/ioController.php',
 
            data:   'path='+pathPython+"&action=readFile",
                ...
                success : function(retorno){
				      ...
				      if(retorno.is_erro == "true"){
					     callWindowMessage("","error","Error",retorno.msg);
						  ...
						  editor =  $('#'+id_Pythoneditor).find('.CodeMirror')[0].CodeMirror;
...
function openPython_O(button,id_program){
...
function savePython(button,id_program){
