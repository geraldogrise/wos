    ...
    function stylechange(arr) {
        ...
        $('#dynamic-styles').text('.dialog h3 {color: ' + x + '} .i-group input:focus ~ label,.i-group input.used ~ label {color: ' + x 
		+ ';} .bar:before,.bar:after {background:' + x + '} .i-group label {color: ' + x + ';} ul.nav > li.active {color:' + x + '} .style-tx {color: ' + 
		x + ';}.style-bg {background:' + x + ';color: white;}@keyframes navgrow {100% {width: 100%;background-color: ' + x 
		+ ';}} ul.list li.context {background-color: ' + x + '}');
    }
    ...
    function setModal(mode, $ctx) {
        ...
        switch (mode) {
            ...
            case 'edit':
				...
				id_contact_list = $ctx.attr('id_contact_list');
				$mod.find('#contactId').val(id_contact_list);
                $mod.find('#new-user').val($ctx.text().trim()).addClass('used');
                break;
        }
    ...
    $('.list-text > ul > li').on('click', function() {
        $('ul.chat > li').eq(1).html('<img src="' + $(this).find('img').prop('src') + '"><div class="message"><p>' + $(this).find('.txt').text() 
		+ '</p></div>');
    ...
    $('.list-account > .list').on('click', 'li', function() {
		...
		id_contact_list = $(this).attr('id_contact_list');
        var $TARGET = $(this);
        if (!$(this).next().hasClass('context')) {
            var $ctx = $('<li class="context"><i id_contact_list ='+id_contact_list+' class="mdi mdi-pencil"></i><i id_contact_list ='+id_contact_list+' class="mdi mdi-delete"></i></li>');
            ...
            $ctx.on('click', '.mdi-pencil', function() {
                ...
                $('#contact-modal').one('click', '.btn.save', function() {
					...
					updateContactList($TARGET);
					
                });
            });

            $ctx.on('click', '.mdi-delete', function() {
                deleteContact(this,$TARGET);
				//$TARGET.remove();
            });
    ...
    $('#add-contact-floater').on('click', function() {
		...
		if ($(this).hasClass('active')) {
            ...
            setModal('add');
            $('#contact-modal').one('click', '.btn.save', function() {
                $('.list-account > .list').prepend('<li><img src="http://lorempixel.com/100/100/people/1/"><span class="name">' 
	...
	$(function(){
	  user = parent.document.getElementById("user_system_id").value;
	   getContact("hangout","contact",user);
