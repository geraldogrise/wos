...
function callWindowJsEditor(id,pathJs){
...
function resizeJsEditor(button){
...
function resizeWindowJsEditor(button,callBackfunction){
    id = $(button).closest('.program').attr('id');
...
function openJsPath(id_Jseditor,pathJs){
  
 

  pathJs=pathJs.replace(/\#/g, "\\");
  $.ajax({
            type      : 'post',
 
            url       :'system/controller/ioController.php',
 
            data:   'path='+pathJs+"&action=readFile",
            ...
            dataType  : 'JSON',
                ...
                success : function(retorno){
				      ...
				      if(retorno.is_erro == "true"){
						  ...
						  editor =  $('#'+id_Jseditor).find('.CodeMirror')[0].CodeMirror;
...
function openJs_O(button,id_program){
...
function saveJs(button,id_program){
